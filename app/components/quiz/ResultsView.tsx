    ...
    <div className="mx-auto w-full max-w-2xl bg-[#1e293b]/80 backdrop-blur-xl p-10 rounded-[2.5rem] border border-white/10 shadow-2xl text-center font-space">
        ...
        <div className="mt-2 mb-6 text-left">
          ...
          <div className="flex flex-wrap gap-2">
            {Array.from(new Set(missed)).map((topic, index) => (
              <span
                key={index}
                className="px-3 py-1.5 bg-rose-500/10 border border-rose-500/20 rounded-lg text-rose-400 text-[10px] font-bold uppercase tracking-tight"
              >
                {topic}
              </span>
            ))}
      ...
      {missedCount > 0 && onRetryMissed && (
        <button
          ...
          className="w-full mb-4 py-4 bg-amber-500/10 hover:bg-amber-500/20 border border-amber-500/30 text-amber-400 font-black rounded-2xl uppercase tracking-widest text-[10px] flex items-center justify-center gap-2 transition-all"
        >
          <RefreshCcw size={14} />

...
import { FireTwist, FaBalanceScale } from "react-icons/fa";
...
export default function FormulaModal({
  ...
  // Create a reference pointer to hook into the scrollable body element
  const scrollContainerRef = useRef<HTMLDivElement>(null);

  // Monitor tab switches to instantly pull the layout back to integer zero
  useEffect(() => {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 antialiased selection:bg-cyan-500/30">
      ...
      {/* Modal Frame Window */}
      <div className="relative z-10 bg-[#1e293b] border border-white/10 w-full max-w-xl rounded-[2.5rem] shadow-[0_25px_50px_-12px_rgba(0,0,0,0.6)] overflow-hidden flex flex-col max-h-[85vh] font-space transform-gpu will-change-transform backface-hidden">
        ...
        {/* SCROLLABLE BODY - SCROLLBAR REMOVED
          Using inline CSS styles to inject webkit scrollbar overrides natively
          so you don't break tailwind workflows!
        */}
        <div 
        ...
        >
          {/* Inject style segment to prune webkit scrollbar tracks entirely */}
          <style>{`
            div::-webkit-scrollbar {
              display: none !important; /* Hides scrollbar on Chrome, Safari, Opera */
            }
          `}</style>

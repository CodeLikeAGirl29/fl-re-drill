...
import { FaDivide, FaPlus, FaMinus, FaEquals } from "react-icons/fa6";
...
export default function Calculator({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
  ...
  return (
    <div className="fixed bottom-24 right-8 z-[70] w-64 bg-[#1e293b] border border-[#444444] rounded-2xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200">
      <div className="bg-slate-800 px-4 py-2 flex justify-between items-center border-b border-white/10 font-space">
      ...
      <div className="p-4 bg-black/20 text-right font-space">
        <div className="text-[0.625rem] text-blue-400 h-4 font-space tracking-tight">{equation}</div>
        <div className="text-2xl font-bold text-white truncate font-space tracking-tighter">
          {display}
        </div>
      </div>
      ...
      <div className="grid grid-cols-4 gap-1 p-2 bg-[#1e293b]">
        ...
        <button onClick={() => handleOperator('/')} className="p-3 bg-blue-600 text-white rounded-lg hover:bg-blue-700 flex justify-center items-center"><FaDivide size={14} /></button>
        ...
        <button onClick={() => handleOperator('×')} className="p-3 bg-blue-600 text-white rounded-lg hover:bg-blue-700 flex justify-center items-center"><FaTimes size={14} /></button>

        {[4, 5, 6].map(n => (
          <button key={n} onClick={() => handleNumber(String(n))} className="p-3 bg-white/5 text-white font-bold rounded-lg hover:bg-white/10 text-lg">{n}</button>
        ))}
        <button onClick={() => handleOperator('-')} className="p-3 bg-blue-600 text-white rounded-lg hover:bg-blue-700 flex justify-center items-center"><FaMinus size={14} /></button>

        {[1, 2, 3].map(n => (
          <button key={n} onClick={() => handleNumber(String(n))} className="p-3 bg-white/5 text-white font-bold rounded-lg hover:bg-white/10 text-lg">{n}</button>
        ))}
        <button onClick={() => handleOperator('+')} className="p-3 bg-blue-600 text-white rounded-lg hover:bg-blue-700 flex justify-center items-center"><FaPlus size={14} /></button>
